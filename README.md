...
**One source means one as-of date.** KPI 5 originally used a second table
(`2ydv-qfge`, the official top-50 ranking) that ends at **2023**. The agent
reported 2025 growth beside a 2023 rank and concluded SFO was "losing ground to
competitors" — reasoning across two different eras. Computing rank from the main
table fixed it, and took coverage from 50 airports to 1,311.
...
- **KPI 3's score is ~90–95% load factor.** `score = lf + cagr + max(gap,0)` sums
  raw units on different scales, so the demand-capacity gap contributes 0–2 points.
  Ranking order is sound; the absolute number is not. Needs normalizing.
- **Rank is computed, not official.** BTS ranks *enplanements*; this ranks *total
  passengers*. Validated against official 2023 at mean 0.1 places (only SEA/MIA
  swap) — close, but not the official figure.
